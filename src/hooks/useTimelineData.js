...
export const useTimelineData = () => {
  ...
  const navigateToTimelineItem = useCallback(async (timelineItem) => {
    if (!timelineItem || timelineItem.id === currentTimelineId) return;
    
    try {
      setIsLoading(true);
      
      // Here you could add additional loading logic if needed
      // For example, preloading images or tooth data
      
      // Simulate a small delay for better UX
      await new Promise(resolve => setTimeout(resolve, 200));
      
      setCurrentTimelineId(timelineItem.id);
      ...
      console.error('Error navigating to timeline item:', err);
      setError(err.message);
      return {
        success: false,
        error: err.message
